...
/*:
 * @plugindesc 変数内の値を万進法表示に変換して表示する機能を追加します。
 * @author jp_asty
 *
 * @param useVariableIds
 * @desc 大きい数値を格納するのに利用する変数IDのリスト
 * @text 変数IDのリスト
 * @type variable[]
 * @default []
 *
 * @param mtDigitNum
 * @desc 無量大数が9999を超えると指数表記になります。そのときの有効桁数です。
 * @text 無量大数の指数表記時の有効桁数
 * @type number
 * @min 1
 * @default 3
 *
 ...
 * ------------------------------------------------------------
 *
 * ※DTextPictureと併用する場合、プラグイン管理でDtextPictureより下にくるようにして下さい。
 * ※このプラグインを有効化するとBigInteger.js(https://www.npmjs.com/package/big-integer)が読み込まれます。
 *
 * 使い方
 * 1.プラグインパラメーターの変数IDのリストに使用する変数のリストを登録します。
 *   ここに登録された変数のみが大きい数字を格納する用の変数として予約されます。
 * 2.変数の操作で対象変数に値を設定します。
 * 3.メッセージウィンドウや動的ピクチャに制御文字\Vm[n1, n2]を使用して表示します。
 * n1:変数のID
 * n2:先頭から表示する桁名の数。0を指定すると全て表示。
 *
 * 例:
 * 変数1に1234005600002345が入っているとき
 * \Vm[1, 0] →　1234兆56億2345
 * \Vm[1, 2] →　1234兆56億
 *
 * ・桁名の後ろの0は省略され、対象桁の値が全てが0のときはその桁名は省略されます。
 * ・値の更新は基本的に変数の操作（加算や乗算など）を通して行って下さい。
...
(function() {
  ...
  const getManshinhoString = function(value, visibleUnitNum) {
    ...
    for(let i=0; i<div4List.length; i++) {
      ...
      if(div4List[i].length > 4) {
        ...
        visibleUnitNum -= Number(eNum / 4);
      }
      outString = outString + div4List[i] + unitNames[div4List.length-i-1];
      visibleUnitNum--;
      if(visibleUnitNum <= 0) break;
    }
    return (value.isNegative() ? '-' : '') + outString;
  };

  const padStartZeroByMultiple4  = function(str) {
    const zeroNum = (4 - (str.length % 4)) % 4;
  ...
  Window_Base.prototype.convertEscapeCharacters = function(text) {
    ...
    text = text.replace(/\x1bVm\[(\d+),\s*(\d+)]/gi, function() {
      const value = $gameVariables.value(parseInt(arguments[1]));
      const visibleUnitNum = parseInt(arguments[2]);
      return getManshinhoString(value, visibleUnitNum);
    }.bind(this));
    return text;
  };
  ...
  Game_Variables.prototype.value = function(variableId) {
    if(useVariableIds.contains(variableId)) {
      return this._data[variableId] || bigInt(0);
    } else {
  ...
  // Game_Interpreter
  //
  const _Game_Interpreter_operateVariable = Game_Interpreter.prototype.operateVariable;
